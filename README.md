...
## Решение
При разработке приложения был выбран язык Go, 
в качестве основной базы данных используется PostgreSQL, 
для кэша используется Redis.
В структуре проекта старался придерживаться стандартного
Go Project-layout, чтобы архитектура сервиса 
была относительно чистой и интуитивной

### Запуск сервера
Для запуска сервера испольуется следующая команда:
```
go run main.go
```

Для запуска сервера в Docker контейнере:
```
docker-compose build
```

Как только имэдж создан можно запускать приложение:
```
docker-compose up
```

При запуске контейнера произойдет иницилизация
базы данных из файла init.sql в папке init, 
конфигурацию для подключения к Postgres и Redis
можно изменить в internal/config/config.go

Так же были написаны тесты для всех путей роутера
